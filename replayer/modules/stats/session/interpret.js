...
function calPasteIns(session) {
  ...
  for (let i = 0; i < ev.length; i++) {
    ...
    if (insLen < medThreshold || delLen > 0) continue;
    ...
    // }
    currentText =
      currentText.slice(0, pos) + ins + currentText.slice(pos + delLen); // update current text
...
function calRevisionIntensity(session) {
  ...
  for (let i = 0; i < ev.length; i++) {
    ...
    if (delLen[i] > 0 && insLen[i] > 0) {
      // replace event
      replacedChars += insLen[i];
    } else if (delLen[i] > 0 && insLen[i] == 0) {
      // pure delete event
  ...
  let currentText =
    init.slice(0, pos[0]) + ins[0] + init.slice(pos[0] + delLen[0]);
  for (let i = 1; i < ev.length; i++) {
    ...
    currentText =
      currentText.slice(0, pos[i]) +
      ins[i] +
      currentText.slice(pos[i] + delLen[i]); // Update displayed text
  ...
  for (let i = 0; i < ev.length; i++) {
    ...
    currentText =
      currentText.slice(0, pos[i]) +
      ins[i] +
      currentText.slice(pos[i] + delLen[i]);
  }
...
function jaccardSim(bigram1, bigram2) {
  const sim =
    bigram1.intersection(bigram2).size /
    Math.max(bigram1.union(bigram2).size, 1);
  return sim;
}
